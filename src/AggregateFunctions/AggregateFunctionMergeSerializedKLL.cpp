...
{
template <typename T>
class AggregationFunctionMergeSerializedKLL final
    : public IAggregateFunctionDataHelper<KLLSketchData<T>, AggregationFunctionMergeSerializedKLL<T>>
{
private:
    bool base64_encoded;  /// If true, data may be base64 encoded and needs decoding check

public:
    AggregationFunctionMergeSerializedKLL(const DataTypes & arguments, const Array & params, bool base64_encoded_)
        : IAggregateFunctionDataHelper<KLLSketchData<T>, AggregationFunctionMergeSerializedKLL<T>>{arguments, params, createResultType()}
        , base64_encoded(base64_encoded_)
    {}

    String getName() const override { return "mergeSerializedKLL"; }
...
AggregateFunctionPtr createAggregateFunctionMergeSerializedKLL(
    const String & name,
    ...
    const Settings *)
{
    ...
    // Use Float64 as template parameter since KLL sketch works with doubles
    return std::make_shared<AggregationFunctionMergeSerializedKLL<Float64>>(argument_types, params, base64_encoded);
}
...
/// mergeSerializedKLL - Merges multiple serialized KLL quantiles sketches into a single sketch
///
/// This function combines multiple KLL sketches that were created by serializedKLL() into
...
/// entire dataset.
///
/// KLL sketches provide better space efficiency (35-60% smaller) than classic quantiles sketches while
/// maintaining formal accuracy guarantees, making them ideal for cross-service data pipelines and
/// distributed analytics.
///
/// Syntax: mergeSerializedKLL([base64_encoded])(sketch_column)
///
/// Parameters (optional):
...
///
/// Arguments:
///   - sketch_column: String - Serialized KLL sketches (from serializedKLL() or previous merges)
///
/// Returns: String
///   A merged serialized KLL sketch. Can be further merged or passed to percentileFromKLL().
///
/// Examples:
...
///
///   -- Merge sketches from Kafka messages (raw binary from Go service)
///   SELECT mergeSerializedKLL(kll_sketch_bytes) FROM kafka_edge_metrics;
///
///   -- Enable base64 decoding for external data
///   SELECT mergeSerializedKLL(1)(sketch) FROM imported_sketches;
///
...
///   - Use base64_encoded=0 (default) for best performance with binary data
...
///   - Cross-service analytics: merge sketches from Kafka, upstream services, or different systems
///   - Time-series percentile analysis with pre-computed sketches
///   - Apache DataSketches format for interoperability with Java, Python, Go implementations
///
/// See also:
///   - serializedKLL() - Create KLL sketches
///   - percentileFromKLL() - Extract percentile from merged sketch
///   - valuesAndWeightsFromKLL() - Extract distribution as JSON
void registerAggregateFunctionMergeSerializedKLL(AggregateFunctionFactory & factory)
{
    AggregateFunctionProperties properties = { .returns_default_when_only_null = true, .is_order_dependent = false };

    factory.registerFunction("mergeSerializedKLL", {createAggregateFunctionMergeSerializedKLL, properties});
}
