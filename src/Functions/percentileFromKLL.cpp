...
class FunctionPercentileFromKLL : public IFunction
{
public:
    static constexpr auto name = "percentileFromKLL";

    static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionPercentileFromKLL>(); }
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        for (size_t i = 0; i < input_rows_count; ++i)
        {
            ...
            try
            {
                /// ClickHouse aggregate functions (serializedKLL, mergeSerializedKLL)
                ...
                auto sketch = datasketches::kll_sketch<double>::deserialize(data_ptr, data_size);
...
REGISTER_FUNCTION(PercentileFromKLL)
{
    FunctionDocumentation::Description description = R"(
Extracts the percentile value from a serialized KLL quantiles sketch.
The function deserializes the KLL sketch and returns the value at the specified percentile.
KLL sketches provide better space efficiency (35-60% smaller) than classic quantiles sketches.
If the input is invalid or empty, returns NaN.
)";
    FunctionDocumentation::Syntax syntax = "percentileFromKLL(serialized_sketch, percentile)";
    FunctionDocumentation::Arguments arguments = {
        {"serialized_sketch", "Serialized KLL sketch as a String.", {"String"}},
    ...
    FunctionDocumentation::Examples examples = {
        {
            ...
            R"(
SELECT percentileFromKLL(serializedKLL(rand()), 0.5) AS median
FROM numbers(1000)
            )",
            ""
        },
        {
            "Get 95th percentile",
            R"(
SELECT percentileFromKLL(serializedKLL(rand()), 0.95) AS p95
FROM numbers(1000)
            )",
            ""
        },
        {
            "Get p50, p95, p99 from merged sketches",
            R"(
SELECT 
...
FROM hourly_latency_sketches
    ...
    factory.registerFunction<FunctionPercentileFromKLL>(documentation);
}
